...
const LicenciasTab = lazy(() => import('./profile/tabs/LicenciasTab'));
const AjustesTab = lazy(() => import('./profile/tabs/AjustesTab'));
const AuditoriaTab = lazy(() => import('./profile/tabs/AuditoriaTab'));
const MarquesinaTab = lazy(() => import('./profile/tabs/MarquesinaTab'));
    ...
    <main className="pt-16 pb-32 px-4 xs:px-5 w-full max-w-md mx-auto min-h-screen overflow-x-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        {userRole?.includes('admin') && activeTab === 'licencias' && (
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 rounded-full border-2 border-[#4be277]/30 border-t-[#4be277] animate-spin" /></div>}>
            <LicenciasTab
              userRole={userRole}
              webDomain={webDomain}
              webSyncFrequency={webSyncFrequency}
              vercelPlan={vercelPlan}
              webDaysRemaining={webDaysRemaining}
              webLicenseActive={webLicenseActive}
              appLicenseActive={appLicenseActive}
              appPwaShortName={appPwaShortName}
              appPushEnabled={appPushEnabled}
              appDaysRemaining={appDaysRemaining}
              newAdminPhone={newAdminPhone}
              setNewAdminPhone={setNewAdminPhone}
              customCodesCount={customCodes.length}
              isLicensingReadOnly={isLicensingReadOnly}
              maintenanceMode={maintenanceMode}
              setShowWebWindow={setShowWebWindow}
              setShowAppWindow={setShowAppWindow}
              setShowDiagnosticsWindow={setShowDiagnosticsWindow}
              setShowMaintenanceWindow={setShowMaintenanceWindow}
              setShowCodesWindow={setShowCodesWindow}
        ...
        {userRole?.includes('admin') && activeTab === 'ajustes' && (
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 rounded-full border-2 border-amber-500/30 border-t-amber-500 animate-spin" /></div>}>
            <AjustesTab
        ...
        {/* VISTA SEGURIDAD / AUDITORÍA (SOLO ADMINS) */}
        {userRole?.includes('admin') && activeTab === 'seguridad' && (
          <Suspense fallback={<div className="flex items-center justify-center py-20"><div className="w-6 h-6 rounded-full border-2 border-blue-500/30 border-t-blue-400 animate-spin" /></div>}>
            <AuditoriaTab
              userRole={userRole}
              newEmail={newEmail}
              newPersonalPhone={newPersonalPhone}
              newPersonalKey={newPersonalKey}
